...
const SavedSkyLinks = ({ viewHome }) => {
	...
	const [skylinkToRemove, setSkylinkToRemove] = useState(null);
	...
	return (
		<>
					...
					<ul>
						{skylinks.map((skylink, index) => (
							<li key={index}>
								...
								<span>
									<button onClick={() => handleConfirmDelete(skylink)}>
			...
			{skylinkToRemove && (
				<ConfirmDeleteModal
					skylinks={skylinks}
					setSkylinks={setSkylinks}
					skylinkToRemove={skylinkToRemove}
					setSkylinkToRemove={setSkylinkToRemove}
				/>
			)}
		</>
	);
